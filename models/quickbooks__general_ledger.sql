...
adjusted_gl as (
    ...
    select distinct
        {{ dbt_utils.generate_surrogate_key(['gl_union.transaction_id', 'gl_union.index', 'gl_union.account_id', ' gl_union.transaction_type', 'gl_union.transaction_source','gl_union.source_relation']) }} as unique_id,
...
final as (
    ...
    select
        *,
        sum(adjusted_amount) over (partition by CONCAT(account_id,source_relation) order by transaction_date, account_id rows unbounded preceding) as running_balance
    from adjusted_gl
)
